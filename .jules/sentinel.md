...
## 2026-03-05 - [Missing CSRF and GET Mutation]
**Vulnerability:** The notification endpoints (`mark_read`, `mark_all_read`, `mark_popup_shown`, `set_reminder`) modified database state but lacked CSRF protections and incorrectly accepted `GET` methods. Because the application utilizes opt-in CSRF (`WTF_CSRF_CHECK_DEFAULT = False`), omitting `csrf.protect()` left these actions entirely open to cross-site request forgery.
**Learning:** Modifying state via GET requests inherently bypasses most standard CSRF protections and allows for trivial exploitation via external <img> or <a> tags. Furthermore, in an opt-in CSRF system, all POST endpoints making modifications must be explicitly annotated.
**Prevention:** Ensure all state-modifying requests require POST methods. Do not assume `csrf.protect()` is automatically active. When fixing server-side routes to strictly require POST, simultaneously convert all frontend reference links (`<a>`) to inline POST `<form>` structures with hidden `csrf_token` inputs, and update any AJAX `fetch` calls to include CSRF headers.

## 2026-03-06 - [Reflected XSS via JavaScript String Interpolation]
**Vulnerability:** A Reflected XSS vulnerability in `superadmin_system_logs.html` occurred because user input (`search_term`) was interpolated directly into a JavaScript string literal inside a `<script>` tag: `if ('{{ search_term }}')`. An attacker could break out of the string literal using a single quote and execute arbitrary JavaScript.
**Learning:** Jinja's default HTML auto-escaping does not protect against XSS when injecting variables directly into a JavaScript context within `<script>` blocks. A variable containing a quote (e.g., `'; alert(1); //`) will bypass HTML escaping and break the JS syntax syntax, leading to code execution.
**Prevention:** Always use the `|tojson` filter when injecting Python variables into JavaScript within templates. This safely serializes the variable as a JSON literal (e.g., `const searchTerm = {{ search_term|tojson }};`), ensuring quotes and special characters are properly escaped for the JS context without breaking out of strings.
