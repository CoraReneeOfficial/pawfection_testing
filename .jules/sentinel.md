...
## 2026-03-05 - [Path Traversal in Backup Download/Delete]
**Vulnerability:** A Path Traversal vulnerability existed in the `/superadmin/backup/download/<filename>` and `/superadmin/backup/delete/<filename>` routes. Although it had a rudimentary `if '..' in filename` check, malicious inputs or different encodings might bypass simpler checks and allow attackers to download or delete arbitrary files on the filesystem.
**Learning:** Checking for `..` manually is insufficient to fully protect against directory traversal, especially across different operating systems or encoded inputs. Filesystem operations taking user input should always sanitize the input thoroughly.
**Prevention:** Always use a robust, established library function for sanitizing user-provided filenames before using them in filesystem operations. In Flask/Werkzeug, `werkzeug.utils.secure_filename` strips out dangerous characters and paths, effectively mitigating path traversal attacks.
