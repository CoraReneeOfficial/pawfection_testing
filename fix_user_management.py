"""
Fix duplicate superadmin_user_management function names in app.py by renaming the second occurrence.
"""
import re
...
def fix_duplicate_function():
    try:
        ...
        matches = list(re.finditer(r'def superadmin_user_management\(\):', content))
        ...
        new_content = (
            ...
            content[start_pos + len("def superadmin_user_management():"):]
        )
        
        # Also change the route path for the second occurrence
        route_pattern = r"@app\.route\('/superadmin/manage-users', methods=\['GET', 'POST'\]\)"
        ...
        if len(route_matches) >= 2:
            ...
            new_content = (
                ...
                "@app.route('/superadmin/manage-users-alt', methods=['GET', 'POST'])" + 
