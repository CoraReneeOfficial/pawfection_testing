...
def add_owner():
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Owner '{name}' added!", "success"); return redirect(url_for('owners.directory'))
        except Exception as e:
            db.session.rollback(); current_app.logger.error(f"Error adding owner: {e}", exc_info=True)
...
def edit_owner(owner_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"Owner '{name}' updated!", "success"); return redirect(url_for('owners.view_owner', owner_id=owner_id))
        except Exception as e:
            db.session.rollback(); current_app.logger.error(f"Error editing owner {owner_id}: {e}", exc_info=True)
